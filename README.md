...
| **日志自动归档** | 调用日志默认保留 30 天（`LOG_RETENTION_DAYS` 可调），超期日志启动时 + 每 24 小时自动压缩为 gzip NDJSON 归档文件（`server/archives/`）后从库中清除；支持手动触发、列表查看与下载 |
...
- **日志归档（`/admin/archives`）**：`LOG_RETENTION_DAYS`（默认 30）天前的调用日志自动导出为 `logs-archive-*.ndjson.gz` 并从库中删除；仅管理员可手动触发 / 列表 / 下载
