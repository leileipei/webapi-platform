...
- 真实网关（`/gw/*`）：注册路径（含 `{param}` 占位符）匹配转发（**路由编译缓存**）、**AccessKey + SecretKey 双因子鉴权**（**AccessKey 内存索引**，SK 哈希常量时间比对）与应用授权校验、QPS 限流、超时 / 失败重试（**仅 GET/PUT/DELETE 等幂等方法自动重试，POST/PATCH 不自动重试防重复提交**）、**熔断保护**（窗口内错误率超阈值自动开启）、调用指标落库、超阈值自动生成告警（QPS 告警按近 60 秒滑动窗口真实 QPS 计算）
