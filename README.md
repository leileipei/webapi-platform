...
- **控制台登录认证**：账号密码登录（**scrypt 加盐哈希**存储，旧 SHA-256 数据登录后自动升级）、**HMAC-SHA256 签名令牌**（12 小时过期，**重启服务不失效**；改密/角色变更/恢复备份后旧令牌立即失效，令牌篡改即拒）、**退出登录令牌进入持久化黑名单**（SQLite 存储哈希，重启后仍失效直至自然过期）、连续 5 次失败锁定 5 分钟、**初始密码首次登录强制修改（服务端硬阻断，错误码 40310）**；`/admin/*` 除登录外均需鉴权，数据面 `/gw/*` 不受影响仍用应用密钥
- **安全加固**：全站安全响应头（CSP / X-Frame-Options / nosniff / Referrer-Policy）；管理接口 CORS 默认不下发（跨域需配置 `CORS_ORIGIN` 白名单）；请求体大小上限（网关默认 10MB，`GATEWAY_MAX_BODY` 可调）；**SSRF 双层防护**（注册 API 时校验 backendUrl + 网关转发时带缓存兜底，云元数据 / 回环 / 链路本地地址拦截，`ALLOW_LOCAL_TEST=1` 可放行本机联调）；**路由 method+path 服务端唯一约束**；**严格生命周期状态机**（草稿→发布→下线→废弃，废弃为终态）；分组引用保护（分组下有 API 禁止删除）；**服务端 Schema 双层校验**（注册时校验 API 定义合法性——方法/路径/认证方式/限流熔断参数范围/分组引用，网关调用时校验必填 Query/Header 参数与 number/boolean 类型、Body JSON 参数的必填与 string/number/boolean/object/array 类型，非法入参返回 40001）
...
- **备份恢复（`/admin/backup`）**：`VACUUM INTO` 生成一致性快照供下载；上传备份文件事务内整体替换业务表（可选含用户表，恢复后注销全部会话），非法文件与缺表备份会被拒绝。**注意：备份含令牌签名密钥与用户表，须按敏感数据保管、仅恢复可信来源**
- 真实网关（`/gw/*`）：注册路径（含 `{param}` 占位符）匹配转发（**路由编译缓存 + 路由表内存快照**，不每请求读库；参数化路由静态段优先匹配，同形路由注册即拒绝）、**AccessKey + SecretKey 双因子鉴权**（**AccessKey 内存索引**，SK 哈希常量时间比对）与应用授权校验、**响应体流式透传**（大文件下载不整报文缓冲，背压自动处理；**上游响应头完整透传**，hop-by-hop 头自动剔除；**指标/日志在流传输完成后才记录**，latency 为全程耗时，中途断流正确记为失败）、QPS 限流、超时 / 失败重试（**仅 GET/PUT/DELETE 等幂等方法自动重试，POST/PATCH 不自动重试防重复提交**）、**熔断保护**（窗口内错误率超阈值自动开启）、调用指标落库（**异步队列 250ms 事务批量写入**，不阻塞数据面）、超阈值自动生成告警（QPS 告警按近 60 秒滑动窗口真实 QPS 计算）
